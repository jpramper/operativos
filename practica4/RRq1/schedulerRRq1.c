...
{
    int cambia_proceso = 0; // bandera de cambio de proceso
    int prox_proceso_a_ejecutar; // pid del proximo proceso a ejecutar
    ...
    if(evento==TIMER) {
      ...
      // si hay elementos en espera, mete el que esta en ejecucion a la cola
      if(!cola_vacia(listos))
      {
        printf("mi cola NOOO esta vacia");
    ...
    if(evento==SOLICITA_E_S)
    {
        ...
        // se mete a la cola de bloqueados
    ...
    if(evento==TERMINA_E_S)
    {
        ...
        // se saca de la cola de bloqueados y se pone en listos
